...
window.openGuidePage = function(filename, pageNum) {
    ...
    if (window.masterActivityDB) {
        for (const act of window.masterActivityDB) {
            if (act.details && act.details.guides) {
                const cleanFilename = filename.replace('.pdf', '').trim();
                const foundGuide = act.details.guides.find(g => 
                    g.name.includes(cleanFilename) || cleanFilename.includes(g.name)
                );
                if (foundGuide) {
                    fileUrl = foundGuide.link;
                    break;
                }
            }
        }
    ...
    if (!fileUrl) {
        alert(`عذراً، لم أتمكن من العثور على رابط الملف لـ "${filename}".`);
        return;
    }
    ...
    if (fileUrl.includes('drive.google.com')) {
        targetUrl = fileUrl.replace('/view', '/preview').replace('/edit', '/preview');
        viewerType = "google_drive";
    } else {
        targetUrl = `https://docs.google.com/viewer?url=${encodeURIComponent(fileUrl)}`;
        viewerType = "gafi_server";
    }
    ...
    // إظهار النافذة مع الزر
    showPdfModal(pageNum, viewerType, targetUrl);
};

// ==================== 🛠️ النافذة الاحترافية مع زر الفتح ====================
function showPdfModal(pageNum, viewerType, targetUrl) {
    ...
    let instruction = "";
    if (viewerType === "google_drive") {
        instruction = `
            <div style="background:rgba(255,255,255,0.1); border-radius:8px; padding:12px; margin:12px 0; text-align:right;">
                <div style="font-size:13px; line-height:2;">
                    ١. سيفتح الملف على Google Drive<br>
                    ٢. تم نسخ رقم الصفحة <strong style="font-size:16px; color:#fbbf24;">(${pageNum})</strong> تلقائياً<br>
                    ٣. في الملف اضغط على حقل الصفحة والصق الرقم
                </div>
            </div>`;
    } else {
        instruction = `
            <div style="background:rgba(255,255,255,0.1); border-radius:8px; padding:12px; margin:12px 0; text-align:right;">
                <div style="font-size:13px; line-height:2;">
                    ١. سيفتح الملف في عارض المستندات<br>
                    ٢. تم نسخ رقم الصفحة <strong style="font-size:16px; color:#fbbf24;">(${pageNum})</strong> تلقائياً<br>
                    ٣. اضغط <strong>Ctrl+G</strong> أو انتقل يدوياً للصفحة
                </div>
            </div>`;
    }
    ...
    modal.innerHTML = `
        <div style="display:flex; align-items:center; gap:12px; margin-bottom:8px;">
            ...
            <div>
                <strong style="font-size:16px; display:block;">الانتقال للصفحة المطلوبة</strong>
                <span style="font-size:12px; opacity:0.8;">كل ما عليك</span>
            </div>
        </div>
...
// ==================== ✂️ فتح التفاصيل في حالة الالتباس ====================
window.showGuideSnippet = function(filename, pageNum, exactPhrase) {
    ...
    // استخراج النص الكامل للصفحة المحددة
    ...
    // إرسال الإجابة مباشرة كرسالة مساعد
