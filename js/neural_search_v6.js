...
window.openGuidePage = function(filename, pageNum) {
    // 1. تنظيف اسم الملف وبناء المسار الكامل من مجلد guides على GitHub
    let cleanName = filename.replace(/\.pdf$/i, '').trim();
    let fileUrl = `guides/${cleanName}.pdf`;

    // 2. التحقق من وجود رابط مباشر في قاعدة البيانات (يعطي الأولوية للرابط المسجل)
    let foundLink = null;
    ...
    // 3. بناء الرابط النهائي — يستخدم الرابط المسجل إن وُجد وإلا يبني من اسم الملف
    let targetUrl = foundLink 
        ? `${foundLink}#page=${pageNum}` 
        : `${fileUrl}#page=${pageNum}`;
    
    let viewerType = "github_direct";

    // 4. نسخ رقم الصفحة للحافظة للاحتياط
    try {
        navigator.clipboard.writeText(pageNum.toString()).catch(e => {});
    } catch (err) {}

    // 5. إظهار النافذة لإرشاد المستخدم
