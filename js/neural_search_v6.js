...
window.openGuidePage = function(filename, pageNum) {
    // 1. تحديد المسار المباشر للملف في مجلد guides
    // نقوم بتنظيف اسم الملف والتأكد من امتداد .pdf
    let cleanName = filename.replace('.pdf', '').trim();
    let fileUrl = `guides/${cleanName}.pdf`;

    // 2. التحقق من وجود الملف في قاعدة البيانات (اختياري للتحقق فقط)
    let foundInDB = false;
    ...
    // 3. بناء الرابط النهائي مع خاصية الانتقال لصفحة محددة
    // هذه الميزة (#page=X) تعمل الآن مباشرة لأن الملف على نفس السيرفر
    let targetUrl = `${fileUrl}#page=${pageNum}`;
    let viewerType = "github_direct"; 

    // 4. نسخ رقم الصفحة للحافظة (للاحتياط أو للاستخدام اليدوي)
    try {
        navigator.clipboard.writeText(pageNum.toString()).catch(e => {});
    } catch (err) {}

    // 5. إظهار النافذة الاحترافية لإرشاد المستخدم
